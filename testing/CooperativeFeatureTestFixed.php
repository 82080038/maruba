<?php
/**
 * Cooperative Feature Testing Framework - Fixed Version
...
class CooperativeFeatureTestFixed
{
    private $testResults = [];
    private $testData = [];
    private $loanData = [];
    ...
    public function testRepaymentFlow($loanId)
    {
        ...
        $loan = $this->getLoan($loanId);
        $expectedBalance = 10000000 - $paymentAmount; // Principal - payment
     ...
     */
    public function testAccountingFlow($loanId, $paymentAmount)
    {
        ...
        $repaymentJournalId = $this->createJournalEntry($loanId, 'repayment');
    ...
    {
        echo "🚀 Starting Cooperative Feature Tests (Fixed Version)\n";
        echo "=====================================\n";
    ...
    private function getLoan($id) { 
        if (!isset($this->loanData[$id])) {
            $this->loanData[$id] = array_merge($this->testData['loan'], ['status' => 'pending', 'balance' => 10000000]);
        }
        return $this->loanData[$id]; 
    }
    private function updateLoanStatus($id, $status) {
        $this->loanData[$id]['status'] = $status;
        return true;
    }
    private function setDisbursementDate($id) {
        $this->loanData[$id]['disbursement_date'] = date('Y-m-d H:i:s');
        return $this->loanData[$id]['disbursement_date'];
    }
    private function updateLoanBalance($id, $payment) {
        $this->loanData[$id]['balance'] -= $payment;
        return $this->loanData[$id]['balance'];
    }
...
if (basename(__FILE__) === basename($_SERVER['SCRIPT_NAME'])) {
    $test = new CooperativeFeatureTestFixed();
    $test->runAllTests();
}
?>
